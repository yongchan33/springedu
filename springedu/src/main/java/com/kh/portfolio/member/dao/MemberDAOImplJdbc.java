...
public class MemberDAOImplJdbc implements MemberDAO {
	...
	public List<MemberVO> listAllMember() {
		...
		list = jdbcTemplate.query(sql.toString(), getRowMapper()); // 자바행과 매핑하는매소드

		return list;
	}

	// 테이블 내용과 자바객체를 매핑하는 방식
	...
	public MemberVO listOneMember(String id) {
		...
		MemberVO memberVO = jdbcTemplate.queryForObject(sql.toString(),
				// 맴버객체와 sql쿼리문과 1:1매칭이 되야댐,
				// 테이블 컴럼 이름과 자바VO객체 속성이름이 동일하면 자동 매핑해줌.
	...
	public int outMember(String id, String pw) {
		logger.info("MemberDAOIMpl.outMember() 호출됨!");
		int result = 0;
	...
	public MemberVO login(String id, String pw) {
		...
		logger.info("MemberDAOIMpl.login() 호출됨!");
		MemberVO memberVO = null;
		StringBuilder sql = new StringBuilder();
		sql.append(" select id,pw,tel,nickname,gender,region,birth,cdate,udate  from member");
		sql.append(" where id= ?  and pw= ? ");

		memberVO = jdbcTemplate.queryForObject(sql.toString(),
				// 맴버객체와 sql쿼리문과 1:1매칭이 되야댐,
				// 테이블 컴럼 이름과 자바VO객체 속성이름이 동일하면 자동 매핑해줌.
	...
	public String findID(String tel, String birth) {
		logger.info("MemberDAOIMpl.findID() 호출됨!");
		...
		id = jdbcTemplate.queryForObject(		//한개의 레코드 받아와서 객체에 저장.
				sql.toString(),
				String.class,
				tel,birth
				);
	...
	public String findPW(String id, String tel, String birth) {
		logger.info("MemberDAOIMpl.findPW() 호출됨!");
		...
		pw = jdbcTemplate.queryForObject(		//한개의 레코드 받아와서 객체에 저장.
				sql.toString(),
				String.class,
				id,tel,birth
				);
	...
	public int changePW(String id, String pw) {
		logger.info("MemberDAOIMpl.changePW() 호출됨!");
		int result = 0;
		StringBuilder sql = new StringBuilder();
		sql.append("update member ");
		sql.append("   set pw 			= ? ");
